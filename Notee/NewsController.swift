...
class NewsController: UIViewController, UIScrollViewDelegate, iCarouselDataSource, iCarouselDelegate, DelegateAlertViewer {
    ...
    func carousel(_ carousel: iCarousel, didSelectItemAt index: Int) {
        if plugs[index].id == "exemple" {
            let viewer = viewerController()
            viewer.plug = plugs[index]
    ...
    func addNewSheet(_ sender : ButtonAddSheet) {
        self.sender = sender
        guard let sheet = sender.sheet else {return}
        if (sheet.isAdded == false){
            let controller = PlugAlertModalView(title: "Ajouter la fiche", description: "Il vous reste \(noteeCoinsAvailables) coins")
            controller.modalPresentationStyle = .overFullScreen
            controller.delegate = self
            controller.currentPlug = sheet
            controller.noteeCoinsAvailables = noteeCoinsAvailables
            self.present(controller, animated: false, completion: {
                controller.addTargetForPlugViewer()
                controller.delegateForBuy = self
                controller.cost = 4
                controller.addTargetForAddSheet()
            })
